class ProductCard extends HTMLElement {
    constructor() {
        ...
        this.name = this.getAttribute("name") ?? "No Data";
        this.totalAmount = this.getAttribute("price") ?? "No Data";
        this.rate = this.getAttribute("rate") ?? "No Data";
        this.views = this.getAttribute("views") ?? "No Data";
        this.count = 1;
        this.#Render();
    }
    ...
    .shop-card-buttons {
      display: flex;
        ...
        <article class="shop-card" onclick="moveToItemDetails('${this.name}')">
        <img src="${this.hotelImg}" class="shop-card-img">
        <h4 class="shop-card-name">${this.name}</h4>
        <h4>${parseInt(this.totalAmount).toLocaleString()}₮</h4>
